...
export default function UnitEconomicsPage() {
  ...
  function addSegment() {
    setInput((prev) => ({
      ...
      segments: [
        ...prev.segments,
        { name: `Segment ${prev.segments.length + 1}`, userPct: 0.1, avgRequestsPerMonth: 50, avgCostPerRequest: 0.06, revenuePerUser: 29 },
      ],
    }));
  }
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    {input.segments.length} segment{input.segments.length !== 1 ? "s" : ""}
                  </CardDescription>
                </div>
                <button
                  onClick={addSegment}
                  className="px-3 py-1.5 text-xs font-medium rounded border border-border text-text-secondary hover:text-text-primary hover:border-border-hover transition-colors"
                >
                  + Add
                </button>
              </div>
